...
import rpg.game.items.ScrollOfAntibioticsItem;
import rpg.game.items.VesselOfMineralsItem;

import java.util.HashMap;
import java.util.List;

public class ScrollOfAntibiotics extends BaseMonster {
    private static final EnumMonsterAlignment alignment = EnumMonsterAlignment.ITEM;
    private final ScrollOfAntibioticsItem item;

    public ScrollOfAntibiotics(double charPosx, double charPosy, Level level) {
        super(charPosx, charPosy, 0, 0, alignment, level);

        preCacheSprites(new HashMap<String, String>() {
            {
                put("idle", "/items/questitems/31.png");
            }
        });
        ...
        this.item = new ScrollOfAntibioticsItem(this, this.getLevel().getPlayer());
