...
import rpg.game.items.ScientificInstrumentItem;
import rpg.game.items.VesselOfMineralsItem;

import java.util.HashMap;
import java.util.List;

public class ScientificInstrument extends BaseMonster {
    private static final EnumMonsterAlignment alignment = EnumMonsterAlignment.ITEM;
    private final ScientificInstrumentItem item;

    public ScientificInstrument(double charPosx, double charPosy, Level level) {
        super(charPosx, charPosy, 0, 0, alignment, level);

        preCacheSprites(new HashMap<String, String>() {
            {
                put("idle", "/items/questitems/12.png");
            }
        });
        ...
        this.item = new ScientificInstrumentItem(this, this.getLevel().getPlayer());
